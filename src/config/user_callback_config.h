 ...
 *
 * @brief Callback functions configuration file - BLE HID gamepad device.
 *
...
#if (BLE_BATT_SERVER)
// Battery Service callbacks (none needed; user_batt_* polls the level itself)
static const struct app_bass_cb user_app_bass_cb = { NULL, NULL };
#endif

#if (BLE_SUOTA_RECEIVER)
static const struct app_suotar_cb user_app_suotar_cb = {
    .on_suotar_status_change = user_ota_status_change,
};
#endif
...
static const struct app_callbacks user_app_callbacks = {
    ...
    .app_on_pairing_succeeded           = user_app_pairing_succeeded,
    .app_on_encrypt_ind                 = user_app_encrypt_ind,
    .app_on_encrypt_req_ind             = user_app_encrypt_req_ind,
...
{
    // HOGPD (HID over GATT - Device role) has no SDK app module, so the application
    // provides the database creation and connection-enable functions.
    {TASK_ID_HOGPD,      app_hogpd_create_db, app_hogpd_enable},
    // Battery Service: the SDK app_bass module is CR2032-only, so this is our own version (HMCLOCK table)
    {TASK_ID_BASS,       user_batt_create_db, user_batt_enable},
