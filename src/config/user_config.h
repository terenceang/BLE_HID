 ...
 * @file user_config.h
 *
 * @brief User configuration file - BLE HID gamepad device.
 *
...
#include "app_version.h"            // APP_VERSION

/// Appearance: Gamepad (Bluetooth assigned number)
#define APP_HID_APPEARANCE_GAMEPAD     (0x03C4)
...
static const struct advertise_configuration user_adv_conf = {
    ...
    .intv_max = MS_TO_BLESLOTS(150),                        // 150ms
 ...
 ****************************************************************************************
 */
/// Advertising data: HID service UUID (0x1812) + Appearance (Gamepad, 0x03C4)
...
/// Scan response data (the device name is appended automatically)
 ...
 ****************************************************************************************
 */
/// Device name is built at runtime instead ("BT-SNES-XXXXXX", XXXXXX = the device's own MAC
/// suffix, so multiple pads are distinguishable) - see user_app_adv_start() in user_hid.c.
/// Left empty so the SDK's automatic name-append (guarded by USER_DEVICE_NAME_LEN > 0) is a no-op.
...
static const struct connection_param_configuration user_connection_param_conf = {
    ...
    /// Keep zero: HID input reports should be delivered without added latency.
    .latency = 0,
...
static const struct default_handlers_configuration  user_default_hnd_conf = {
    ...
    // A HID device must pair and encrypt right after connection.
    // Possible values:
    //  - DEF_SEC_REQ_NEVER
 ...
 * Just Works pairing: bonding + LE Secure Connections, no MITM.
 ************************************************************
 */
#define USER_CFG_FEAT_AUTH_REQ  (GAP_AUTH_BOND | GAP_AUTH_SEC)

/************************************************************
 * Encryption Max key size (7 to 16)
 ************************************************************
 */
